use {Bingo, Goal, Template, Mode};
use seed_random::SeedRandom;

static LINE_CHECK_LIST: &'static [&'static [usize]; 25] =
...
pub fn generate(seed: u32, mode: Mode, template: &Template) -> Bingo {
    let mut random = SeedRandom::new(seed.to_string().bytes());

    let mut cells = <[[String; 5]; 5]>::default();

    // populate the bingo board in the array
    let mut gen_cells = Vec::with_capacity(25);
    for i in 1..26 {
    ...
    for (c, g) in cells.iter_mut().flat_map(|r| r.iter_mut()).zip(gen_cells.into_iter()) {
        *c = g.goal.name.clone();
    }
